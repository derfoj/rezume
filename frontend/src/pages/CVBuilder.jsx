...
const INITIAL_LOGS = [
  { id: 'init-1', text: "Initializing Orchestrator...", type: 'success' },
  { id: 'init-2', text: "System ready. Waiting for input stream...", type: 'info' }
];
...
export default function CVBuilderApp() {
  const { token, logout, user, theme, profileData } = useAuth();
      ...
      pollInterval = setInterval(async () => {
        try {
          ...
          if (data.status === 'success') {
            ...
            addToast("CV généré avec succès !", "success");
            
            const finalUrl = `${API_URL}/api/download/${jobId}?token=${token}`;
            const inlineUrl = `${API_URL}/api/download/${jobId}?token=${token}&inline=true`;
            ...
            if (isDownloading) {
                // Better download trigger (avoids pop-up blocker)
            ...
            setIsPreviewing(false);
            setIsDownloading(false);
            addLog(`Error detected by worker. Please check your data.`, 'error');
            addToast("La génération a échoué.", "error");
            setJobId(null);
            clearInterval(pollInterval);
          } else {
            addLog(`Background worker: ${data.status}...`, 'wait');
          }
        } catch (err) {
  ...
  }, [jobId, isGenerating, isPreviewing, isDownloading, token, API_URL, addToast]);
  ...
  const startGeneration = async (mode) => {
    ...
    setLogs([]); 
    addLog(`Initiating ${mode} generation...`, 'info');
    addLog("Sending specialized request to worker queue...", 'process');
    ...
    try {
      ...
      addLog(`Job #${data.job_id} accepted. Monitoring status...`, 'wait');
              ...
              <div className="flex gap-2">
                <button 
                  onClick={() => {
                    ...
                    link.download = 'reZume_CV.pdf';
                    link.click();
                  }}
                  className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700 font-bold btn-interactive"
                >
                  Télécharger
                </button>
      ...
      <div className="w-full mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 h-full">
        ...
        <div className="col-span-1 lg:col-span-3 glass-panel p-6 flex flex-col items-center text-center h-auto">
          <div className="relative mb-6">
            <div className="w-24 h-24 rounded-full border-2 border-blue-200 dark:border-blue-900 p-1 bg-white dark:bg-slate-800 flex items-center justify-center overflow-hidden shadow-lg">
              {user?.avatar_image?.startsWith('http') || user?.avatar_image?.startsWith('uploads/') ? (
                <img
                  src={user.avatar_image.startsWith('http') ? user.avatar_image : `${API_URL}/data/img/${user.avatar_image}`}
                  alt="Avatar"
          ...
          <p className="text-xs tech-accent mb-8 font-bold tracking-wider">{user?.title || 'Candidat'}</p>
              ...
              <div className="glass-panel p-6 flex-1 animate-fadeIn flex flex-col bg-white/60 dark:bg-slate-900/40 overflow-hidden">
                <div className="flex items-center justify-between mb-4 border-b border-gray-100 dark:border-slate-800 pb-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => startGeneration('download')}
                      disabled={isGenerating}
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar space-y-4">
                  ...
                  {results.skills && results.skills.length > 0 && (
                    <div className="p-3 bg-purple-50/50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-900/30 rounded text-left">
        ...
        <div className="col-span-1 lg:col-span-3 flex flex-col gap-6 h-auto">
          ...
          <div className="glass-panel p-6 h-1/2 flex flex-col justify-center items-center text-center bg-slate-50/50 dark:bg-slate-900/20 border-dashed relative">
            ...
            <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1 px-4 text-center">API Endpoint: <code className="bg-gray-100 dark:bg-slate-800 px-1 rounded text-blue-500 dark:text-blue-400">POST /api/analyze</code></p>
          </div>
        </div>
